...
import { Building2, Search, Filter, ArrowRight, FileText, Users, Sparkles } from 'lucide-react';
...
export interface WholesaleCategoryPageProps {
  ...
  relatedCategoryLinks?: { title: string; path: string; description: string }[];
}

export const WholesaleCategoryPage: React.FC<WholesaleCategoryPageProps> = ({
  title,
  h1,
  subtitle,
  badge = 'UK Wholesale Category Catalog',
  metaTitle,
  metaDescription,
  canonicalPath,
  breadcrumbs,
  ...
  }, [categoryProducts, searchQuery, selectedGrade]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10 bg-[#FAF8F2]">
        ...
        <div className="bg-white rounded-2xl border border-[#D8E2DE] p-6 sm:p-10 b2b-card-shadow space-y-4">
          <div className="border-b border-[#D8E2DE] pb-3">
            <span className="text-xs font-bold text-[#00A88F] uppercase tracking-wider">Wholesale Category Overview</span>
            <h2 className="text-2xl font-black text-[#071715] mt-1">{title}</h2>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-[#D8E2DE] b2b-card-shadow flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative w-full sm:w-80">
            ...
            <input
              type="text"
              placeholder="Search category stock..."
              value={searchQuery}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-extrabold text-[#071715]">
              Available Category Stock ({filteredProducts.length})
            </h3>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-[#D8E2DE] p-8 text-center space-y-3 b2b-card-shadow">
              <p className="text-sm font-semibold text-[#596662]">
                No specific models matched your current filter criteria.
              </p>
              <p className="text-xs text-[#596662]">
                We continually receive new wholesale shipments. Contact our trade desk to request live unlisted stock updates.
              </p>
              <Link to="/contact">
        ...
        {relatedCategoryLinks.length > 0 && (
          <div className="space-y-4">
            <h3 className="text-lg font-extrabold text-[#071715]">Related Wholesale Categories & Brands</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {relatedCategoryLinks.map((rel, idx) => (
                <Link
                  key={idx}
                  to={rel.path}
